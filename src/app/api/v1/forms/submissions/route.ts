...
export async function GET(request: Request) {
  ...
  const usage = await checkApiUsageLimit(auth.userId, "forms", auth.scopes);
  if (!usage.allowed) {
    const message =
      usage.reason === "no_plan"
        ...
        ? "This API key isn't scoped for the Forms API. Create a new key with that scope, or use an unscoped key."
