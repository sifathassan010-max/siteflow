...
export async function POST(request: Request) {
  ...
  const usage = await checkApiUsageLimit(auth.userId, "seo", auth.scopes);
  if (!usage.allowed) {
    const message =
      usage.reason === "no_plan"
        ...
        ? "This API key isn't scoped for the SEO API. Create a new key with that scope, or use an unscoped key."
